...
Command-line style task list in the browser. Static **HTML / CSS / JS** only — open **`tasks.html`** (double-click works). State lives in **`localStorage`**.

| File | Purpose |
|------|---------|
| `tasks.html` | Page |
| `tasks.css` | Styles |
| `tasks.js` | Logic; **`HELP_PANEL_HTML`** powers the **`help`** popup |

**Docs:** this README is the full reference. The in-app **`help`** text (`HELP_PANEL_HTML` in **`tasks.js`**) is separate — short hints only; change it when you want the popup updated.
...
| **settings** | Opens the **settings** panel (same centered layout as **help** / **finished**). Toggle **Require `add` keyword**; preference is saved in **`localStorage`**. |
